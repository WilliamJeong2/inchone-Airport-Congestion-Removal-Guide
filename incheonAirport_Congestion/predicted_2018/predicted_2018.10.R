...
#해당 숫자 변경하여 이용 현재 data10 = 10월
data10$월요일<-gsub(",","",data10$월요일) #월요일의 3자리 단위로 있는 ,를 삭제
data10$월요일<-as.numeric(data10$월요일) #화요일를 numeric로
df_mon3<-data.frame(일자=data10$요일,월요일=data10$월요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data10$화요일<-gsub(",","",data10$화요일) #월요일의 3자리 단위로 있는 ,를 삭제
data10$화요일<-as.numeric(data10$화요일) #화요일를 numeric로
df_tue2<-data.frame(일자=data10$요일,화요일=data10$화요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data10$수요일<-gsub(",","",data10$수요일) #월요일의 3자리 단위로 있는 ,를 삭제
data10$수요일<-as.numeric(data10$수요일) #화요일를 numeric로
df_wed3<-data.frame(일자=data10$요일,수요일=data10$수요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data10$목요일<-gsub(",","",data10$목요일) #월요일의 3자리 단위로 있는 ,를 삭제
data10$목요일<-as.numeric(data10$목요일) #화요일를 numeric로
df_thu3<-data.frame(일자=data10$요일,목요일=data10$목요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data10$금요일<-gsub(",","",data10$금요일) #월요일의 3자리 단위로 있는 ,를 삭제
data10$금요일<-as.numeric(data10$금요일) #화요일를 numeric로
df_fri3<-data.frame(일자=data10$요일,금요일=data10$금요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data10$토요일<-gsub(",","",data10$토요일) #월요일의 3자리 단위로 있는 ,를 삭제
data10$토요일<-as.numeric(data10$토요일) #화요일를 numeric로
df_sat3<-data.frame(일자=data10$요일,토요일=data10$토요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data10$일요일<-gsub(",","",data10$일요일) #월요일의 3자리 단위로 있는 ,를 삭제
data10$일요일<-as.numeric(data10$일요일) #화요일를 numeric로
df_sun2<-data.frame(일자=data10$요일,일요일=data10$일요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

#인러닝 값 대입
#2018년 10월 월요일~일요일 
in_learning_mon<-getDataSet(df_mon10$월요일,1,15,INPUT_NODES) #10월 값들 입력
in_learning_tue<-getDataSet(df_tue10$화요일,1,15,INPUT_NODES) #10월 값들 입력
in_learning_wed<-getDataSet(df_wed10$수요일,1,15,INPUT_NODES) #10월 값들 입력
in_learning_thu<-getDataSet(df_thu10$목요일,1,15,INPUT_NODES) #10월 값들 입력
in_learning_fri<-getDataSet(df_fri10$금요일,1,15,INPUT_NODES) #10월 값들 입력
in_learning_sat<-getDataSet(df_sat10$토요일,1,15,INPUT_NODES) #10월 값들 입력
in_learning_sun<-getDataSet(df_sun10$일요일,1,15,INPUT_NODES) #10월 값들 입력

#아웃러닝 값 대입
#2018년 10월 월요일~일요일
out_learning_mon<-getDataSet(df_mon10$월요일,3,16,OUTPUT_NODES) #10월 값들 출력
out_learning_tue<-getDataSet(df_tue10$화요일,3,16,OUTPUT_NODES) #10월 값들 출력
out_learning_wed<-getDataSet(df_wed10$수요일,3,16,OUTPUT_NODES) #10월 값들 출력
out_learning_thu<-getDataSet(df_thu10$목요일,3,16,OUTPUT_NODES) #10월 값들 출력
out_learning_fri<-getDataSet(df_fri10$금요일,3,16,OUTPUT_NODES) #10월 값들 출력
out_learning_sat<-getDataSet(df_sat10$토요일,3,16,OUTPUT_NODES) #10월 값들 출력
out_learning_sun<-getDataSet(df_sun10$일요일,3,16,OUTPUT_NODES) #10월 값들 출력

#신경망 모델 중 하나인 nnet 패키지 함수를 이용하여 10월~110월 이용자 수 예측값 모델링 설정
# 2018년 10월 월요일~일요일
...
in_forecasting_mon<-getDataSet(df_mon10$월요일,15,16,INPUT_NODES)
in_forecasting_tue<-getDataSet(df_tue10$화요일,15,16,INPUT_NODES)
in_forecasting_wed<-getDataSet(df_wed10$수요일,15,16,INPUT_NODES)
in_forecasting_thu<-getDataSet(df_thu10$목요일,15,16,INPUT_NODES)
in_forecasting_fri<-getDataSet(df_fri10$금요일,15,16,INPUT_NODES)
in_forecasting_sat<-getDataSet(df_sat10$토요일,15,16,INPUT_NODES)
in_forecasting_sun<-getDataSet(df_sun10$일요일,15,16,INPUT_NODES)

#1~12월 이용자 수 예측값 출력
#2018년 10월 월요일~일요일 예측값 출력
predicted_values_mon10<-predict(model_mon,in_forecasting_mon,type="raw")
predicted_values_tue10<-predict(model_tue,in_forecasting_tue,type="raw")
predicted_values_wed10<-predict(model_wed,in_forecasting_wed,type="raw")
predicted_values_thu10<-predict(model_thu,in_forecasting_thu,type="raw")
predicted_values_fri10<-predict(model_fri,in_forecasting_fri,type="raw")
predicted_values_sat10<-predict(model_sat,in_forecasting_sat,type="raw")
predicted_values_sun10<-predict(model_sun,in_forecasting_sun,type="raw")
