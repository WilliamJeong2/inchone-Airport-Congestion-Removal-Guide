...
df_tue2<-data.frame(일자=data8$요일,화요일=data8$화요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경
...
#인러닝 값 대입
#2018년 8월 월요일~일요일 
in_learning_mon<-getDataSet(df_mon8$월요일,1,15,INPUT_NODES) #8월 값들 입력
in_learning_tue<-getDataSet(df_tue8$화요일,1,15,INPUT_NODES) #8월 값들 입력
in_learning_wed<-getDataSet(df_wed8$수요일,1,15,INPUT_NODES) #8월 값들 입력
in_learning_thu<-getDataSet(df_thu8$목요일,1,15,INPUT_NODES) #8월 값들 입력
in_learning_fri<-getDataSet(df_fri8$금요일,1,15,INPUT_NODES) #8월 값들 입력
in_learning_sat<-getDataSet(df_sat8$토요일,1,15,INPUT_NODES) #8월 값들 입력
in_learning_sun<-getDataSet(df_sun8$일요일,1,15,INPUT_NODES) #8월 값들 입력

#아웃러닝 값 대입
#2018년 8월 월요일~일요일
out_learning_mon<-getDataSet(df_mon8$월요일,3,16,OUTPUT_NODES) #8월 값들 출력
out_learning_tue<-getDataSet(df_tue8$화요일,3,16,OUTPUT_NODES) #8월 값들 출력
out_learning_wed<-getDataSet(df_wed8$수요일,3,16,OUTPUT_NODES) #8월 값들 출력
out_learning_thu<-getDataSet(df_thu8$목요일,3,16,OUTPUT_NODES) #8월 값들 출력
out_learning_fri<-getDataSet(df_fri8$금요일,3,16,OUTPUT_NODES) #8월 값들 출력
out_learning_sat<-getDataSet(df_sat8$토요일,3,16,OUTPUT_NODES) #8월 값들 출력
out_learning_sun<-getDataSet(df_sun8$일요일,3,16,OUTPUT_NODES) #8월 값들 출력
...
in_forecasting_mon<-getDataSet(df_mon8$월요일,15,16,INPUT_NODES)
in_forecasting_tue<-getDataSet(df_tue8$화요일,15,16,INPUT_NODES)
in_forecasting_wed<-getDataSet(df_wed8$수요일,15,16,INPUT_NODES)
in_forecasting_thu<-getDataSet(df_thu8$목요일,15,16,INPUT_NODES)
in_forecasting_fri<-getDataSet(df_fri8$금요일,15,16,INPUT_NODES)
in_forecasting_sat<-getDataSet(df_sat8$토요일,15,16,INPUT_NODES)
in_forecasting_sun<-getDataSet(df_sun8$일요일,15,16,INPUT_NODES)

#1~12월 이용자 수 예측값 출력
#2018년 8월 월요일~일요일 예측값 출력
predicted_values_mon8<-predict(model_mon,in_forecasting_mon,type="raw")
predicted_values_tue8<-predict(model_tue,in_forecasting_tue,type="raw")
predicted_values_wed8<-predict(model_wed,in_forecasting_wed,type="raw")
predicted_values_thu8<-predict(model_thu,in_forecasting_thu,type="raw")
predicted_values_fri8<-predict(model_fri,in_forecasting_fri,type="raw")
predicted_values_sat8<-predict(model_sat,in_forecasting_sat,type="raw")
predicted_values_sun8<-predict(model_sun,in_forecasting_sun,type="raw")
