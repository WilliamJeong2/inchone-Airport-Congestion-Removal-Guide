...
data11<-read.xlsx2(file.choose(),1) #2001년~2017년 19월 데이터

data12<-read.xlsx2(file.choose(),1) #2001년~2017년 19월 데이터
...
###############세부 적으로 2018년 9월~19월 각 달의 요일 별 이용자 수 예측 #############

# 2018년 9월 월요일~일요일 사용자 수 예측

#해당 숫자 변경하여 이용 현재 data9 = 9월
data9$월요일<-gsub(",","",data9$월요일) #월요일의 3자리 단위로 있는 ,를 삭제
data9$월요일<-as.numeric(data9$월요일) #화요일를 numeric로
df_mon3<-data.frame(일자=data9$요일,월요일=data9$월요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data9$화요일<-gsub(",","",data9$화요일) #월요일의 3자리 단위로 있는 ,를 삭제
data9$화요일<-as.numeric(data9$화요일) #화요일를 numeric로
df_tue2<-data.frame(일자=data9$요일,화요일=data9$화요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data9$수요일<-gsub(",","",data9$수요일) #월요일의 3자리 단위로 있는 ,를 삭제
data9$수요일<-as.numeric(data9$수요일) #화요일를 numeric로
df_wed3<-data.frame(일자=data9$요일,수요일=data9$수요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data9$목요일<-gsub(",","",data9$목요일) #월요일의 3자리 단위로 있는 ,를 삭제
data9$목요일<-as.numeric(data9$목요일) #화요일를 numeric로
df_thu3<-data.frame(일자=data9$요일,목요일=data9$목요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data9$금요일<-gsub(",","",data9$금요일) #월요일의 3자리 단위로 있는 ,를 삭제
data9$금요일<-as.numeric(data9$금요일) #화요일를 numeric로
df_fri3<-data.frame(일자=data9$요일,금요일=data9$금요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data9$토요일<-gsub(",","",data9$토요일) #월요일의 3자리 단위로 있는 ,를 삭제
data9$토요일<-as.numeric(data9$토요일) #화요일를 numeric로
df_sat3<-data.frame(일자=data9$요일,토요일=data9$토요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

data9$일요일<-gsub(",","",data9$일요일) #월요일의 3자리 단위로 있는 ,를 삭제
data9$일요일<-as.numeric(data9$일요일) #화요일를 numeric로
df_sun2<-data.frame(일자=data9$요일,일요일=data9$일요일) # 요일->일자로, 월요일->월요일로 컬럼 이름 변경

#인러닝 값 대입
#2018년 9월 월요일~일요일 
in_learning_mon<-getDataSet(df_mon9$월요일,1,15,INPUT_NODES) #9월 값들 입력
in_learning_tue<-getDataSet(df_tue9$화요일,1,15,INPUT_NODES) #9월 값들 입력
in_learning_wed<-getDataSet(df_wed9$수요일,1,15,INPUT_NODES) #9월 값들 입력
in_learning_thu<-getDataSet(df_thu9$목요일,1,15,INPUT_NODES) #9월 값들 입력
in_learning_fri<-getDataSet(df_fri9$금요일,1,15,INPUT_NODES) #9월 값들 입력
in_learning_sat<-getDataSet(df_sat9$토요일,1,15,INPUT_NODES) #9월 값들 입력
in_learning_sun<-getDataSet(df_sun9$일요일,1,15,INPUT_NODES) #9월 값들 입력

#아웃러닝 값 대입
#2018년 9월 월요일~일요일
out_learning_mon<-getDataSet(df_mon9$월요일,3,16,OUTPUT_NODES) #9월 값들 출력
out_learning_tue<-getDataSet(df_tue9$화요일,3,16,OUTPUT_NODES) #9월 값들 출력
out_learning_wed<-getDataSet(df_wed9$수요일,3,16,OUTPUT_NODES) #9월 값들 출력
out_learning_thu<-getDataSet(df_thu9$목요일,3,16,OUTPUT_NODES) #9월 값들 출력
out_learning_fri<-getDataSet(df_fri9$금요일,3,16,OUTPUT_NODES) #9월 값들 출력
out_learning_sat<-getDataSet(df_sat9$토요일,3,16,OUTPUT_NODES) #9월 값들 출력
out_learning_sun<-getDataSet(df_sun9$일요일,3,16,OUTPUT_NODES) #9월 값들 출력

#신경망 모델 중 하나인 nnet 패키지 함수를 이용하여 9월~19월 이용자 수 예측값 모델링 설정
# 2018년 9월 월요일~일요일
...
in_forecasting_mon<-getDataSet(df_mon9$월요일,15,16,INPUT_NODES)
in_forecasting_tue<-getDataSet(df_tue9$화요일,15,16,INPUT_NODES)
in_forecasting_wed<-getDataSet(df_wed9$수요일,15,16,INPUT_NODES)
in_forecasting_thu<-getDataSet(df_thu9$목요일,15,16,INPUT_NODES)
in_forecasting_fri<-getDataSet(df_fri9$금요일,15,16,INPUT_NODES)
in_forecasting_sat<-getDataSet(df_sat9$토요일,15,16,INPUT_NODES)
in_forecasting_sun<-getDataSet(df_sun9$일요일,15,16,INPUT_NODES)

#1~19월 이용자 수 예측값 출력
#2018년 9월 월요일~일요일 예측값 출력
predicted_values_mon9<-predict(model_mon,in_forecasting_mon,type="raw")
predicted_values_tue9<-predict(model_tue,in_forecasting_tue,type="raw")
predicted_values_wed9<-predict(model_wed,in_forecasting_wed,type="raw")
predicted_values_thu9<-predict(model_thu,in_forecasting_thu,type="raw")
predicted_values_fri9<-predict(model_fri,in_forecasting_fri,type="raw")
predicted_values_sat9<-predict(model_sat,in_forecasting_sat,type="raw")
predicted_values_sun9<-predict(model_sun,in_forecasting_sun,type="raw")
